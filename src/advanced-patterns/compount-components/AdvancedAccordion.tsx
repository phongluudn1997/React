...
type AdvancedAccordionProps = React.PropsWithChildren<{ defaultOpen?: boolean }>
...
const useAccordionContext = () => {
    ...
    if (!context) {
        throw new Error("Accordion components must be used within an AdvancedAccordion")
    }
    return context
}

const AdvancedAccordion = ({children, defaultOpen = false}: AdvancedAccordionProps) => {
    const [openItems, setOpenItems] = useState(defaultOpen ? [0] : [])
    const toggleItem = (index: number) => {
        setOpenItems(openItems => openItems.includes(index) ? openItems.filter((item) => index === item) : [...openItems, key])
    }
    const isOpen = (index: number) => {
        return openItems.includes(index)
    }
    return <AdvancedAccordionContext.Provider value={{isOpen, toggleItem}}>
        {Children.map(children, (child, index) => cloneElement(child, {key: index, index}))}
    </AdvancedAccordionContext.Provider>
}

const AdvancedAccordionItem = ({children, index}: AdvancedAccordionItemProps) => {
...
const AdvancedAccordionHeader = ({children, index}: AdvancedAccordionItemProps) => {
    const {isOpen, toggleItem} = useAccordionContext()
    return <button onClick={() => toggleItem(index)}>
        <span>{children}</span>
        {isOpen(index) ? <ChevronDown/> : <ChevronRight/>}
    </button>
}

const AdvancedAccordionContent = ({children, index}: AdvancedAccordionItemProps) => {
